 ...
 * @author:LiuZhouLiang
 * 
 * @time:2015-3-23下午3:31:46
 * 
 */
public class MyUserEvaluationNew extends BaseActivity implements
		OnMoreListener, OnRefreshListener {
	private static final String TAG = "AppointmentOrder";
	private MyUserEvaluationAdapterNew evaluationAdapter;
	...
	protected void loadChildView() {
		...
		tvData = (TextView) findViewById(R.id.my_userevaluation_date);
	}
	...
	protected void getNetData() {
		...
		map.put("yyyyMMString", TimeUtil.getCurrentDate());
		if (pageNum == 0) {
			/**
			 * 加载第一页
			 */
		...
		if (pageNum == 0) {
			...
			if (evaluateEntity != null && evaluateEntity.getCode() == 0) {
				...
				if (evaluateEntity.getInfo() != null
					...
					startCount = evaluateEntity.getInfo().getEndCount() + 1;
					evaluateDetailList = evaluateEntity.getInfo()
							.getEvaluateList();
					evaluationAdapter = new MyUserEvaluationAdapterNew(
							evaluateDetailList, baseContext);
					listview.setAdapter(evaluationAdapter);
					tvData.setText(TimeUtil.getCurrentDateC());
					LogUtil.d(TAG, "页数" + pageNum + "startCount==" + startCount
			...
			if (evaluateEntity != null && evaluateEntity.getCode() == 0) {
				...
				if (evaluateEntity.getInfo() != null
					...
					if (evaluationAdapter != null) {
						evaluationAdapter.setNewData(evaluateDetailList);
					}
